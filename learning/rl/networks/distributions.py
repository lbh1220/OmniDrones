...
class DiagGaussian_tanh(nn.Module):
    def __init__(self, num_inputs, num_outputs, min_std=0.1, max_std=0.5):
        super(DiagGaussian_tanh, self).__init__()
    ...
    def forward(self, x):
        ...
        action_logstd = self.logstd(zeros)
        # 限制标准差的范围
        std = action_logstd.exp().clamp(self.min_std, self.max_std)
