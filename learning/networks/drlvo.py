...
#
class DrlvoFeaturesExtractor(nn.Module):
    ...
    #
    def __init__(self, observation_space: gym.spaces.Box, features_dim:int = 256, **kwargs):
        super().__init__()
        ...
        self.vector_input_dim = observation_space["robot_node"].shape[-1]
        self.linear_fc = nn.Sequential(
            nn.Linear(256 * block.expansion + self.vector_input_dim, features_dim),
    ...
    def forward(self, observations) -> torch.Tensor:
        # preprocessing:
        ped_pos = observations["ped_map"]
        # scan = observations[:, 12800:19200]
        goal = observations["robot_node"]
        fusion_in = ped_pos
