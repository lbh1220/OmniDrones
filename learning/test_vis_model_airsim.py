...
def main():
    ...
    parser.add_argument("--grid_res", type=float, default=5.0, help="Grid resolution in meters")
    ...
    # ==== AirSim-style observation comparison (single-env) ====
    try:
        from types import SimpleNamespace
        # Import AirSim-side observation processor (standalone-friendly copy)
        from compare_airsim import IsaacLikeObservationProcessor as AirsimObsProc  # type: ignore

        # Pick one env for comparison
        env_idx = 0

        # Build AirSim-like config matching Isaac cfg/processor
        total_traffic_num = int(base_env.obs_processor.total_traffic_num)
        area_size = max(
            base_env.cfg.traffic_sim.area_bounds.xmax - base_env.cfg.traffic_sim.area_bounds.xmin,
            base_env.cfg.traffic_sim.area_bounds.ymax - base_env.cfg.traffic_sim.area_bounds.ymin,
        )
        circle_radius = float(area_size / 2.0 * (2 ** 0.5))
        airsim_cfg = SimpleNamespace(
            observation_radius=float(base_env.cfg.observation_radius),
            vehicle_name="ego",
            max_aircraft_in_obs=total_traffic_num,
            obs_noise_level=0.0,
            use_global_planner=bool(base_env.cfg.use_global_path),
            predict_steps=int(base_env.cfg.predict_steps),
            pred_timestep=float(base_env.cfg.pred_timestep),
        )
        sb3_cfg = SimpleNamespace(
            obs=SimpleNamespace(
                use_angle_distance_obs=bool(getattr(base_env.cfg, "use_angle_distance_obs", False)),
                norm_scale=float(base_env.cfg.observation_norm_scale),
                circle_radius=circle_radius,
            )
        )

        airsim_proc = AirsimObsProc(config=airsim_cfg, sb3_config=sb3_cfg)

        # Build AirSim-like EnvironmentState
        ego_pos = state.ego_drone.drone_state[env_idx, 0, :3].detach().cpu().numpy()
        ego_vel = state.ego_drone.drone_state[env_idx, 0, 7:10].detach().cpu().numpy()
        ego_radius = float(base_env.cfg.safety_radius)
        ego_v_pref = float(base_env.cfg.v_pref)

        vehicle = SimpleNamespace(
            radius=ego_radius,
            max_velocity=ego_v_pref,
            pose=SimpleNamespace(
                position=SimpleNamespace(x=float(ego_pos[0]), y=float(ego_pos[1]), z=float(ego_pos[2])),
                orientation=SimpleNamespace(to_array=lambda: np.array([0.0, 0.0, 0.0, 1.0], dtype=np.float32)),
            ),
            get_position_array=lambda: np.array([float(ego_pos[0]), float(ego_pos[1]), float(ego_pos[2])], dtype=np.float32),
            get_velocity_array=lambda: np.array([float(ego_vel[0]), float(ego_vel[1]), float(ego_vel[2])], dtype=np.float32),
            get_yaw=lambda: 0.0,
        )

        goal_xyz = state.navigation.target_positions[env_idx, 0, :3].detach().cpu().numpy()
        if base_env.cfg.use_global_path:
            local_goal_xyz = state.navigation.local_goals[env_idx, 0, :3].detach().cpu().numpy()
            proj_xyz = state.navigation.projection_points[env_idx, 0, :3].detach().cpu().numpy()
        else:
            local_goal_xyz = goal_xyz
            proj_xyz = ego_pos

        navigation = SimpleNamespace(
            goal_position=SimpleNamespace(to_array=lambda: goal_xyz.astype(np.float32)),
            local_goal_position=SimpleNamespace(to_array=lambda: local_goal_xyz.astype(np.float32)),
            closest_point_on_path=SimpleNamespace(to_array=lambda: proj_xyz.astype(np.float32)),
        )

        # Build raw_aircraft_states from Isaac traffic tensors
        tp = traffic_pos.detach().cpu().numpy() if traffic_pos is not None else np.zeros((0, 3), dtype=np.float32)
        tv = traffic_vel.detach().cpu().numpy() if traffic_vel is not None else np.zeros((0, 3), dtype=np.float32)
        tt = traffic_types.detach().cpu().numpy() if traffic_types is not None else np.zeros((0,), dtype=np.int64)
        tr = traffic_radii.detach().cpu().numpy() if traffic_radii is not None else np.zeros((0,), dtype=np.float32)
        raw_aircraft_states = []
        for i in range(tp.shape[0]):
            raw_aircraft_states.append(
                SimpleNamespace(
                    name=f"aircraft_{i}",
                    aircraft_type="evtol" if int(tt[i]) == 2 else "uav",
                    position=SimpleNamespace(x=float(tp[i, 0]), y=float(tp[i, 1]), z=float(tp[i, 2])),
                    linear_velocity=SimpleNamespace(x=float(tv[i, 0]), y=float(tv[i, 1]), z=float(tv[i, 2])),
                    radius=float(tr[i]),
                )
            )

        airsim_state = SimpleNamespace(
            vehicle=vehicle,
            navigation=navigation,
            raw_aircraft_states=raw_aircraft_states,
        )

        airsim_obs = airsim_proc.process_observation(airsim_state)

        # Extract single-env Isaac obs for comparison
        isaac_single = {
            'robot_node': observations['robot_node'][env_idx].detach().cpu().numpy(),
            'temporal_edges': observations['temporal_edges'][env_idx].detach().cpu().numpy(),
            'spatial_edges': observations['spatial_edges'][env_idx].detach().cpu().numpy(),
            'visible_masks': observations['visible_masks'][env_idx].detach().cpu().numpy(),
            'spatial_types': observations['spatial_types'][env_idx].detach().cpu().numpy(),
        }

        # Pretty print both sides for manual inspection
        print("\n=== Dump Isaac vs AirSim observation (env 0) ===")

        def dump_field(name: str, a: np.ndarray, b: np.ndarray):
            print(f"\n[{name}]")
            print(f"  Isaac shape: {a.shape}")
            print(f"  AirSim shape: {b.shape}")
            print(f"  Isaac values:\n{a}")
            print(f"  AirSim values:\n{b}")

        # Scalars/1D/2D small fields
        dump_field('robot_node', isaac_single['robot_node'], np.asarray(airsim_obs['robot_node']))
        dump_field('temporal_edges', isaac_single['temporal_edges'], np.asarray(airsim_obs['temporal_edges']))

        # Spatial edges and related masks/types per row
        se_isaac = isaac_single['spatial_edges']
        se_airsim = np.asarray(airsim_obs['spatial_edges'])
        vm_isaac = isaac_single['visible_masks']
        vm_airsim = np.asarray(airsim_obs['visible_masks'])
        st_isaac = isaac_single['spatial_types']
        st_airsim = np.asarray(airsim_obs['spatial_types'])

        print("\n[spatial_edges]")
        print(f"  Isaac shape: {se_isaac.shape}")
        print(f"  AirSim shape: {se_airsim.shape}")
        rows = max(se_isaac.shape[0], se_airsim.shape[0]) if se_isaac.ndim == 2 and se_airsim.ndim == 2 else 0
        for i in range(rows):
            isaac_row = se_isaac[i] if i < se_isaac.shape[0] else None
            airsim_row = se_airsim[i] if i < se_airsim.shape[0] else None
            print(f"  row {i:02d} Isaac:")
            print(isaac_row)
            print(f"  row {i:02d} AirSim:")
            print(airsim_row)

        print("\n[visible_masks]")
        print(f"  Isaac shape: {vm_isaac.shape}")
        print(f"  AirSim shape: {vm_airsim.shape}")
        max_mask = max(vm_isaac.shape[0], vm_airsim.shape[0])
        for i in range(max_mask):
            v_i = float(vm_isaac[i]) if i < vm_isaac.shape[0] else None
            v_a = float(vm_airsim[i]) if i < vm_airsim.shape[0] else None
            print(f"  idx {i:02d}: Isaac={v_i}, AirSim={v_a}")

        print("\n[spatial_types]")
        print(f"  Isaac shape: {st_isaac.shape}")
        print(f"  AirSim shape: {st_airsim.shape}")
        max_types = max(st_isaac.shape[0], st_airsim.shape[0])
        for i in range(max_types):
            t_i = int(st_isaac[i]) if i < st_isaac.shape[0] else None
            t_a = int(st_airsim[i]) if i < st_airsim.shape[0] else None
            print(f"  idx {i:02d}: Isaac={t_i}, AirSim={t_a}")
    except Exception as e:
        print(f"[AirSim compare skipped] {e}")
    # 模型预测（无env闭环，仅单步）
