...
def main():
    ...
    parser.add_argument("--model_dir", type=str, 
                        default="runs/traffic/future_penalty/u10e1_r8.0/split_attn/f0",
    ...
    parser.add_argument("--focus_near_traffic", action="store_true", default=True, help="Sample near traffic instead of uniform grid")
    ...
    traffic_pos, traffic_vel, traffic_types, traffic_radii = generate_random_traffic(
        cfg,
        torch.device("cuda"),
    )
    ...
    try:
        ...
        def compare_arrays(a: np.ndarray, b: np.ndarray, name: str, atol=1e-5, rtol=1e-5):
            ...
            print(f"[diff] {name} max_abs_err={diff[idx]:.6g} at {idx}: {a[idx]} vs {b[idx]}")
            return False

        print("\n=== Compare Isaac vs AirSim observation (env 0) ===")
        keys = ['robot_node', 'temporal_edges', 'spatial_edges', 'visible_masks', 'spatial_types']
        all_ok = True
        for k in keys:
            a = isaac_single[k]
            b = np.asarray(airsim_obs[k])
            all_ok &= compare_arrays(a, b, k)
