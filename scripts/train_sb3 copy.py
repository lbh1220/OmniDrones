...
def create_env(cfg, headless=True):
    ...
    # 设置headless模式
    if headless:
        cfg.sim.device = "cuda" if torch.cuda.is_available() else "cpu"
...
def main():
    ...
    try:
        ...
        # 创建评估环境（使用较少的环境数量）
        eval_cfg = NavEnvCfg()
        eval_cfg.scene.num_envs = min(64, args.num_envs // 4)  # 使用较少的环境进行评估
        eval_cfg.drone_model = args.drone_model
        eval_cfg.num_actions = 2
        eval_cfg.num_observations = 7
        eval_env = create_env(eval_cfg, headless=True)
